...
export default function ReportCard() {
  ...
  const performLRGM = (data: any[]) => {
    if (data.length < 2) return "Insufficient data for detailed LRGM analysis. Continue regular assessments to build a growth profile.";
    
    // Group by exam and calculate average percentages
    ...
    if (examAverages.length < 2) return "Showing steady performance. More data points needed for trend analysis.";

    // Simple Linear Regression Slope (m)
    ...
    const growthPct = Math.round(slope * 10); // Scaled for readability

    if (slope > 0.5) {
      return `Growth trend identified! Performance is improving significantly. Your consistency has improved by approximately ${growthPct}% since the start of the year.`;
    } else if (slope < -0.5) {
      return `A slight downward trend detected in recent exams. We recommend focusing on consistency in core subjects to maintain the desired grade level.`;
    } else {
      return `Performance is exceptionally stable. Maintaining a steady grade baseline across ${examAverages.length} examination terms with minimal variance.`;
    }
  };
  ...
  const fetchReport = useCallback(async () => {
    ...
    // Fetch ALL data for the year for LRGM
    ...
    setLoading(false);
  }, [selectedChild, selectedExam, selectedYear]);
