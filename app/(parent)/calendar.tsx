...
const SCHOOL_HOLIDAYS: Record<string, { day: number; label: string }[]> = {
  ...
  '2026-8': [
    { day: 15, label: 'Independence Day' },
  ],
  '2026-10': [
    { day: 2, label: 'Gandhi Jayanti' },
    { day: 12, label: 'Dussehra' },
    { day: 31, label: 'Diwali' },
  ],
  '2026-11': [
    { day: 1, label: 'Diwali Holiday' },
  ],
  '2026-1': [
    { day: 26, label: 'Republic Day' },
  ],
};
...
const SCHOOL_EVENTS: Record<string, { day: number; label: string; color: string }[]> = {
  ...
  '2026-7': [
    { day: 20, label: 'Orientation Day', color: '#6366F1' },
  ],
  '2026-8': [
    { day: 10, label: 'Inter-School Quiz', color: '#F59E0B' },
  ],
  '2026-12': [
    { day: 15, label: 'Annual Day', color: '#EC4899' },
    { day: 20, label: 'Christmas Celebration', color: '#10B981' },
  ],
};

const FEE_TERMS = [
  { term: 'Term 1 (Apr–Sep)', dueDate: 'Jun 15, 2026', amount: '₹25,000', status: 'pending' as const },
  { term: 'Term 2 (Oct–Mar)', dueDate: 'Nov 15, 2026', amount: '₹25,000', status: 'upcoming' as const },
];
...
export default function CalendarPage() {
  ...
  const { data: dbCalendarData } = useCalendarData(selectedChild?.id, year, month);
                ...
                if ((row === 0 && col < firstDayOfWeek) || dayCounter > daysInMonth) {
                  ...
                  cells.push(
                    <View key={col} style={[
                      ...
                      exam && { backgroundColor: '#FEE2E2', borderRadius: 8 }, // Light red background for exams
            ...
            <Card key={i} variant="outlined" style={styles.listCard}>
              <View style={styles.feeRow}>
                <View style={[styles.feeIcon, {
                  backgroundColor: fee.status === 'pending' ? theme.warningLight : theme.borderLight
                }]}>
                  <CreditCard size={18} color={fee.status === 'pending' ? theme.warning : theme.textMuted} />
                </View>
                <View style={{ flex: 1 }}>
...
const styles = StyleSheet.create({
  ...
  calendarCell: {
    flex: 1, aspectRatio: 1, alignItems: 'center', justifyContent: 'center', margin: 1,
  },
